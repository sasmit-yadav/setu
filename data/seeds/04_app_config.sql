...
INSERT INTO app_config (key, value, unit, note) VALUES
  ...
  ('ml.http_timeout_s',          '25',   'seconds', 'API→HF Space /translate and /embed timeout. 8s was below the real cost: a WARM dist-200M answers a headline in 5.8s and a full sentence in 8.5s on this laptop, so every live compose timed out and cached nothing, and the Kerala gate then blocked the Send. 25s clears a warm call with headroom; it does NOT cover a cold container (36.3s incl. lazy model load), so warm the model before a demo. Dispatch never waits on this path for cached alerts'),
  ('ml.target_langs',            'ml',   'csv', 'Languages pre-cached for case-study delivery surfaces when the ML service is reachable. mr was dropped: it is not read by anyone in Kerala, yet it cost 2 of the 6 sequential model calls on every compose (~11s of a 56s Save as draft). Maharashtra still gets mr from _case_study_langs, which keys off the alert geometry rather than a global list'),
...
INSERT INTO app_config (key, value, unit, note) VALUES
  ...
  ('demo.password_emails', 'vythiri.a@setu.example,vythiri.b@setu.example,state.admin@setu.example,auditor@setu.example,relay.node@setu.example,citizen@setu.example', 'csv',
     'Accounts that receive SETU_DEMO_PASSWORD at provision time. Hash only; password stays in .env.'),
  ('demo.unit_scope.vythiri.a@setu.example', 'Vythiri', 'name',
     'geoBoundaries ADM3 containing Muttil North / Meppadi. Wayanad is not an ADM3/ADM5 name.'),
  ('demo.unit_scope.vythiri.b@setu.example', 'Vythiri', 'name',
